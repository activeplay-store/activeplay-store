...
function CoverImage({ src, alt, title }: { src: string; alt: string; title: string }) {
  ...
  if (failed) {
    return (
      <div className="w-full h-full bg-gradient-to-br from-[#1a1a3e] to-[#0a0a2e] flex items-center justify-center p-4">
        <span className="text-white/70 text-sm font-medium text-center leading-tight">{title}</span>
      </div>
    );
  }
  return (
      ...
      {looped.map((game, i) => (
        <div key={i} className="flex-shrink-0 w-[180px] sm:w-[200px] group cursor-pointer">
          <div className="relative rounded-xl overflow-hidden aspect-[3/4] transition-transform duration-300 group-hover:scale-105 group-hover:shadow-lg group-hover:shadow-black/40">
            <CoverImage src={game.image} alt={`${game.title} — EA Play${label ? ' ' + label : ''}`} title={game.title} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent pointer-events-none" />
            <p className="absolute bottom-2 left-2 right-2 text-sm font-medium text-white pointer-events-none">{game.title}</p>
          </div>
        </div>
      ))}
    </div>
