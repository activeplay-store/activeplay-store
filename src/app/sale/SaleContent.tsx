...
const steps = [
  { num: '01', title: 'Выберите игру', desc: 'Найдите нужную игру на странице скидок и нажмите "Купить"' },
  { num: '02', title: 'Напишите нам', desc: 'Telegram, VK или чат на сайте — менеджер ответит за 2 минуты' },
  { num: '03', title: 'Оплатите через СБП', desc: 'Переведите сумму через Систему быстрых платежей — без комиссий' },
  { num: '04', title: 'Играйте через 5 мин', desc: 'Мы активируем игру на вашем аккаунте PS5/PS4. Готово!' },
];
...
export default function SaleContent({ faqItems }: SaleContentProps) {
  ...
  return (
    <>
      ...
      {/* 2. How to buy */}
      <ScrollReveal>
        <section className="py-12 sm:py-16 px-4 sm:px-6" style={{ background: 'rgba(10,21,37,0.5)' }}>
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {steps.map((step, i) => (
                <div key={step.num} className={`relative p-6 rounded-xl backdrop-blur-sm ${i === 3 ? 'shadow-[0_0_40px_rgba(0,212,255,0.1)]' : ''}`} style={{ background: 'rgba(15,23,42,0.5)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <span className="text-5xl font-bold gradient-text font-rajdhani opacity-50 block mb-3">{step.num}</span>
                  <h3 className="text-lg font-semibold text-white mb-2 font-rajdhani">{step.title}</h3>
      ...
      <section className="relative z-10 py-12 sm:py-16 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <a href="https://t.me/activeplay1" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 min-h-[48px] py-3 rounded-xl font-semibold text-white btn-telegram hover:brightness-110">
            ...
            <a href="https://vk.com/activeplay" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 min-h-[48px] py-3 rounded-xl font-semibold text-white btn-vk hover:brightness-110">
