...
ЗАПРЕТ ВЫДУМЫВАНИЯ. Если в поле description/title исходной статьи НЕТ:
 — конкретной даты релиза
 — оценки Metacritic
 — перечисления платформ
то ты НЕ имеешь права подставлять эти факты из своей памяти, даже если знаешь.
Верни для таких полей null. Лучше меньше фактов, чем ложные.
Приоритет истины: (1) description исходника, (2) RAWG metadata, (3) твоя память — только если п.1 и п.2 молчат И ты уверен на 100%.` },
