...
ARC Raiders Flashpoint предлагает миссии против армии роботов в полуоткрытых локациях. Игроки выбирают один из нескольких классов, собирают лут и прокачивают снаряжение. Бои сочетают стрельбу с тактическим позиционированием. Разработчики обещают регулярные обновления и новый контент каждый сезон.
...
// ═══ Генерация с валидацией и retry (до 3 попыток с обратной связью) ═══
async function generateFullArticle(article, enrichedContext) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (validation.valid) {
        console.log(`[NEWS] Article passed validation on attempt ${attempt + 1} (${parsed.site.text.length} chars)`);
      } else {
        console.log(`[NEWS] Attempt ${attempt + 1} failed validation: ${validation.errors.join(', ')}`);
        lastResult = parsed;
        if (attempt < MAX_RETRIES) continue;
        console.warn(`[NEWS] All retries exhausted, using last result (${parsed.site.text.length} chars)`);
      }
